    ...
    for (j = 0; j < len; j++) {
        ...
        for (i = 0; i < 300 - 2; i += 2)
            acf[j] += (spectrum[i] * cos(param * j * i) +
                       4 * spectrum[i + 1] * cos(param * j * (i + 1)) + spectrum[i + 2] * cos(param * j * (i + 2)));
    }
    ...
    for (j = 0; j < len; j++) {
        ...
        for (i = 0; i < num_of_harm - 2; i += 2) {
            acf[j] += (spectrum[i] * cos(paramj * i) +
                       4 * spectrum[i + 1] * cos(paramj * (i + 1)) + spectrum[i + 2] * cos(paramj * (i + 2)));
        }
    }
    ...
    for (j = 0; j < len; j++) {
        ...
        for (i = 0; i < num_of_harm - 2; i += 2) {
            acf[j] += spectrum[i] * COS[i][j] +
                      4 * spectrum[i + 1] * COS[i + 1][j] + spectrum[i + 2] * COS[i + 2][j];
        }
    }
...
int acf_library_light_short_conv(FILE *f, int *temperatures, int temperatures_len, double ti, double te, double *acf, int len) {
    ...
    ti_int = ((int)(floor(ti)) / 20) * 20;  // округл¤ем ti
    te_int = ((int)(floor(te)) / 20) * 20;  // округл¤ем te
...
int acf_library_oxygen(FILE *f, int *temperatures, int temperatures_len, double ti, double te, double *acf, int len) {
    ...
    */
    ...
    ti_int = ((int)(floor(ti)) / 10) * 10;  // округл¤ем ti
    te_int = ((int)(floor(te)) / 10) * 10;  // округл¤ем te
...
int acf_library_heavy(FILE *f, int *temperatures, int temperatures_len, double ti, double te, double *acf, int len) {
    ...
    if (len > 27)  // в библиотеках хран¤тс¤ 27 точек ј ‘
        len = 27;
